...
import sys
sys.path.append("/Users/rdchlkrd/Desktop/krd/CoastSat.Arctic/coastsat_arctic")

import SDS_tools_arctic
...
from local_coastseg.coastseg import common, exceptions
from local_coastseg.coastseg.validation import get_satellites_in_directory
from local_coastseg.coastseg.filters import filter_model_outputs, apply_land_mask
from local_coastseg.coastseg.common import get_filtered_files_dict, edit_metadata, get_files_dict
...
def process_satellite(
    ...
    **kwargs: dict,
    ...
    filepath = get_filepath(settings["inputs"], "S2")
    filepath = filepath[0].replace("S2", "S1").replace("ms", "VH")
    ...
    for batch in range(num_batches):
        ...
        for index in range(
            ...
            geoaccuracy_list.append("PASSED") #"metadata[satname]["acc_georef"][index]")
...
def process_satellite_image(
    ...
    data_S1 = gdal.Open(file_S1)
    bands_sar = [data_S1.GetRasterBand(k + 1).ReadAsArray() for k in range(data_S1.RasterCount)]
    im_S1_2d = bands_sar[0]
    im_S1 = np.repeat(im_S1_2d[:, :, np.newaxis], 3, axis=2)
    georef = np.array(data_S1.GetGeoTransform())
    cloud_cover = 0
    # from skimage.io import imread
    # im_S1 = imread(filename)
    # im_s1 = 
    ...
    ref_shoreline_buffer = SDS_shoreline.create_shoreline_buffer(land_mask.shape, georef, image_epsg,
                                                    pixel_size, settings)

    im_nodata = np.full(np.shape(land_mask), False)
    ...
    if np.shape(im_S1[:,:,0]) != np.shape(im_nodata):
        # create dictionary of output
        print(f'{filename} failed because of a size issue')
        # output = {
...
def create_overlay(
    ...
    combined_float = im_RGB * (1 - overlay_opacity) + overlay[:,:,0] * overlay_opacity
...
def shoreline_detection_figures(
    ...
    # im_RGB = im_ms
    array_normalized = (im_ms - im_ms.min()) / (im_ms.max() - im_ms.min())  # Normalize to 0-1
    im_RGB = array_normalized #* 255  # Scale to 0-255
    im_RGB = im_RGB[:,:,0]

    im_merged = create_overlay(im_RGB, merged_labels, overlay_opacity=0.7)
    im_all = create_overlay(im_RGB, all_labels, overlay_opacity=0.7)
...
def extract_shorelines_with_dask(
    ...
    **kwargs: dict,
    ...
    """
    sitename = settings["inputs"]["sitename"]
    # s1_sitename = settings["s1_sitename"]
    filepath_data = settings["inputs"]["filepath"]
    import glob
    s1_sitename = glob.glob(os.path.join(filepath_data, sitename, "S1", "meta", "*.txt"))[0].split("S1")[-1].split("VH")[0][1:-1]
    ...
    filtered_files = get_files_dict(session_path + "/good", "npz", sitename, s1_sitename)
...
def get_sorted_model_outputs_directory(
    ...
    satellites = ['S1']  #TODO Integrate into above
...
class Extracted_Shoreline:
    ...
    def create_extracted_shorelines_from_session(
        ...
        **kwargs: dict,
        ...
        metadata_s1 = SDS_tools_arctic.get_metadata_s1(self.shoreline_settings["inputs"])
    ...
    def extract_shorelines(
        ...
        # gets metadata used to extract shorelines
        metadata = get_metadata(self.shoreline_settings["inputs"])
        metadata_s1 = SDS_tools_arctic.get_metadata(self.shoreline_settings["inputs"])
        metadata = metadata_s1
        sitename = self.shoreline_settings["inputs"]["sitename"]
        filepath_data = self.shoreline_settings["inputs"]["filepath"]
