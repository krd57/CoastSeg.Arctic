...
from local_coastseg.coastseg import common
from local_coastseg.coastseg import downloads
from local_coastseg.coastseg import sessions
from local_coastseg.coastseg import extracted_shoreline
from local_coastseg.coastseg import geodata_processing
from local_coastseg.coastseg import file_utilities
from local_coastseg.coastseg import core_utilities
...
def get_imagery_directory(img_type: str, RGB_path: str) -> str:
    ...
    # default filetype is NIR and if NDWI is selected else filetype to SWIR
        ...
        # SWIR_path = os.path.join(output_path, "SWIR")
        # output_path = RGB_to_infrared(RGB_path, SWIR_path, output_path, "preprocessed_s1")
        output_path = output_path.replace("preprocessed", "preprocessed_s1")
        print(output_path)
    elif img_type == "SAR_4BAND":
        # SWIR_path = os.path.join(output_path, "SWIR")
        # output_path = RGB_to_infrared(RGB_path, SWIR_path, output_path, "preprocessed_s1")
        output_path = output_path.replace("preprocessed", "preprocessed_s1_4band")
        print(output_path)
    else:
        raise ValueError(
